...
public class ExternalInventorySystem {
    ...
    public ExternalInventorySystem(){
        try {
            ...
            while(scnr.hasNextLine()){
                ...
                product = new Product(Integer.parseInt(temp[3]), Double.parseDouble(temp[0]),
                        Double.parseDouble(temp[1]), temp[2], row);
     ...
     */
    public Product fetchProduct(int id) throws ProductNotFoundException, DatabaseNotFoundException {
        if(id == 1) throw new DatabaseNotFoundException("Database not found, try reconnecting", new Exception());
        try {
            return currentInventory.get(id);
        }catch (IndexOutOfBoundsException e){
            throw new ProductNotFoundException(id, "Barcode is incorrect, could not find item with identifier " + id );
        }
    }
